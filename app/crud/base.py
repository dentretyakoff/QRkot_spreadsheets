...
from app.core.config import DEFAULT_INVESTED_STATUS
...
class CRUDBase:
    ...
    async def get_opened(self, session: AsyncSession):
        """Получает список открытых объектов."""
        db_obj = await session.execute(
            select(self.model)
            .where(self.model.fully_invested == DEFAULT_INVESTED_STATUS)
